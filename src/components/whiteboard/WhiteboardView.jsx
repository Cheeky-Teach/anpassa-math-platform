...
const WhiteboardView = ({ onBack, lang }) => {
    ...
    const updateDivisions = (id, delta) => {
        setElements(prev => prev.map(el => el.id === id ? { ...el, divisions: Math.max(1, (el.divisions || 1) + delta), filledIndices: [] } : el));
    };
    ...
    const toggleFill = (id, idx) => {
        setElements(prev => prev.map(el => {
            if (el.id === id) {
                ...
                if (filled.includes(idx)) filled.splice(filled.indexOf(idx), 1); else filled.push(idx);
    ...
    const handleMouseDown = (e) => {
        ...
        // --- INSTANT SPAWN TOOLS ---
        if (['timer', 'clock', 'ruler'].includes(activeTool)) {
        ...
        if (activeTool === 'coord') { newEl.stepX = 1; newEl.stepY = 1; newEl.gridSize = 40; newEl.isFirstQuadrant = false; newEl.showLabels = true; newEl.fontSize = 20; }
        else if (activeTool === 'line') { newEl.x2 = x; newEl.y2 = y; newEl.showEquation = false; }
    ...
    const handleMouseUp = () => { 
        ...
        setElements(current => {
            const filtered = current.filter(el => {
                ...
                if (['timer', 'clock', 'ruler'].includes(el.type)) return true;
    ...
    const renderElement = (el) => {
        ...
        if (el.type === 'ruler') {
            ...
            for (let i = 0; i <= rng + 0.01; i += step) {
                const val = el.min + i, xp = el.x + i * pxU;
                ...
                <g key={el.id} transform={transform} onClick={(e) => { e.stopPropagation(); setSelectedId(el.id); }}>
                    ...
                    {el.showGraph && (
                        <g transform={`translate(${el.x + tableW + 60}, ${el.y + el.height - 90})`}>
                            ...
                            {yTicks}
                            {el.rows.map((row, i) => {
                                const barWidth = (graphW / el.rows.length) * 0.7, barH = (row.value / maxVal) * graphH;
        ...
        if (el.type === 'timer') {
            const r = el.width / 2, cx = el.x + r, cy = el.y + r;
            const prog = (el.timeLeft / el.duration) * 360, isDone = el.timeLeft === 0;
                        ...
                        <foreignObject x={cx - r*0.9} y={cy} width={r*1.8} height={r} className="ui-ignore">
                            <div className="flex flex-col items-center gap-3">
                                <div className="flex gap-4">
                                    <button onClick={(e)=>{ e.stopPropagation(); setElements(p=>p.map(o=>o.id===el.id?{...o, isRunning:!o.isRunning}:o)); }} className={`p-5 rounded-full shadow-2xl ${el.isRunning ? 'bg-amber-500':'bg-emerald-500'} text-white`}><Play size={32}/></button>
                                    <button onClick={(e)=>{ e.stopPropagation(); setElements(p=>p.map(o=>o.id===el.id?{...o, timeLeft:el.duration, isRunning:false}:o)); }} className="p-5 bg-white rounded-full shadow-xl text-slate-600 border"><RefreshCw size={32}/></button>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={(e)=>{ e.stopPropagation(); setElements(p=>p.map(o=>o.id===el.id?{...o, duration:o.duration+60, timeLeft:o.timeLeft+60}:o)); }} className="px-5 py-2.5 bg-white/90 rounded-2xl shadow-md text-sm font-black">+1m</button>
                                    <button onClick={(e)=>{ e.stopPropagation(); setElements(p=>p.map(o=>o.id===el.id?{...o, duration:Math.max(0,o.duration-60), timeLeft:Math.max(0,o.timeLeft-60)}:o)); }} className="px-5 py-2.5 bg-white/90 rounded-2xl shadow-md text-sm font-black">-1m</button>
                                </div>
                            </div>
                        </foreignObject>
                    )}
        ...
        if (el.type === 'protractor') {
            ...
            for (let i = 0; i <= 180; i += 1) {
                ...
                ticks.push(<line key={i} x1={el.x + r + (r-l)*Math.cos(-a)} y1={el.y + r + (r-l)*Math.sin(-a)} x2={el.x + r + r*Math.cos(-a)} y2={el.y + r + r*Math.sin(-a)} stroke="black" strokeWidth={i % 10 === 0 ? 3 : 1} />);
                if (i % 10 === 0) ticks.push(<text key={`t-${i}`} x={el.x+r+(r-45)*Math.cos(-a)} y={el.y+r+(r-45)*Math.sin(-a)} textAnchor="middle" fontSize="16" fontWeight="900" fill="black">{i}</text>);
            }
            return <g key={el.id} transform={transform} onClick={e=>{e.stopPropagation(); setSelectedId(el.id);}}><path d={`M ${el.x} ${el.y+r} A ${r} ${r} 0 0 1 ${el.x+el.width} ${el.y+r} Z`} fill="white" fillOpacity="0.5" stroke="black" strokeWidth="2" />{ticks}<circle cx={el.x+r} cy={el.y+r} r="5" fill="black" />{showUI && renderHandles(el, r)}</g>;
        }
        ...
        if (el.type === 'shapes_3d') {
            ...
            if (el.shape3D === 'cube' || el.shape3D === 'prism') {
                ...
                if (el.showInternal) lines.push(<path key="h1" d={`M ${el.x} ${el.y+d+h} L ${el.x+d} ${el.y+h} L ${el.x+d} ${el.y} M ${el.x+d} ${el.y+h} L ${el.x+w+d} ${el.y+h}`} fill="none" {...dotted} />);
            } else if (el.shape3D === 'triprism') {
                const bX = el.x + d, bY = el.y;
                faces.push(<path key="back" d={`M ${bX} ${bY+h} L ${bX+w-d} ${bY+h} L ${bX+(w-d)/2} ${bY+d} Z`} {...common} strokeDasharray="4"/>);
                ...
                faces.push(<path key="h1" d={`M ${el.x} ${el.y+h/2} L ${el.x+w} ${el.y+h/2} L ${el.x+w} ${el.y+h} L ${el.x} ${el.y+h} Z`} {...common} />);
                ...
                if (el.shape3D === 'silo') faces.push(<path key="dome" d={`M ${el.x} ${el.y+d/2} A ${w/2} ${w/2} 0 0 1 ${el.x+w} ${el.y+d/2}`} {...common} fillOpacity={0.4} transform={`translate(0, ${-d/4}) rotate(180, ${el.x+w/2}, ${el.y+d/2})`} />);
        ...
        // Standard Circle/Rect/Node/Spinner...
        ...
        if (['rect', 'frac_rect', 'circle', 'frac_circle', 'spinner', 'node'].includes(el.type)) {
            if (el.divisions) {
                for (let i = 0; i < el.divisions; i++) {
                    if (el.type.includes('rect')) {
                        ...
                        fills.push(<rect key={i} x={el.x + (i * sw)} y={el.y} width={sw} height={el.height} fill={el.filledIndices?.includes(i) ? el.stroke : 'transparent'} className="cursor-pointer" onClick={(e) => { e.stopPropagation(); toggleFill(el.id, i); }} />);
                        ...
                        fills.push(<path key={i} d={d} fill={el.filledIndices?.includes(i) ? el.stroke : (el.type==='spinner'?'white':'transparent')} className="cursor-pointer" onClick={(e) => { e.stopPropagation(); toggleFill(el.id, i); }} />);
            ...
            return (
                <g key={el.id} transform={transform} onMouseEnter={() => setHoveredId(el.id)} onMouseLeave={() => setHoveredId(null)} onClick={(e) => { e.stopPropagation(); setSelectedId(el.id); }} onDoubleClick={() => el.type === 'node' && setEditingId(el.id)}>
                    ...
                    {borderL}
                    {el.type === 'node' && editingId !== el.id && <text x={cx} y={cy+10} textAnchor="middle" className="text-2xl font-black fill-black select-none pointer-events-none">{el.label}</text>}
                    {el.showLabel && <text x={el.type.includes('rect') ? el.x + el.width/2 : cx} y={el.type.includes('rect') ? el.y - 25 : cy - r - 25} textAnchor="middle" className="text-3xl font-black fill-black select-none pointer-events-none">{el.filledIndices?.length || 0}/{el.divisions}</text>}
                    {showUI && renderHandles(el, r)}
                </g>
            );
        }
    ...
    const renderHandles = (el, radius = 0) => {
        ...
        return (
            <g className="ui-ignore">
                ...
                <foreignObject x={el.x} y={botY+20} width={el.type === 'tchart' ? 600 : 480} height={350}>
                    <div className="flex flex-wrap gap-4 bg-white rounded-2xl shadow-2xl border-2 border-emerald-500 p-5 pointer-events-auto text-[14px] font-black uppercase items-center">
                        {el.type === 'tchart' && (<>
                            <div className="flex flex-col gap-3 w-full">
                                <div className="flex gap-2">
                                    <button onClick={()=>{ setElements(p=>p.map(o=>o.id===el.id?{...o, rows: [...o.rows, {label: '?', value:0}]}:o)); }} className="px-3 py-2 bg-emerald-100 rounded-lg text-xs font-black uppercase">+ Rad</button>
                                    ...
                                    <button onClick={()=>setElements(p=>p.map(o=>o.id===el.id?{...o, showGraph: !o.showGraph}:o))} className={`px-3 py-2 rounded-lg text-xs ${el.showGraph ? 'bg-emerald-600 text-white':'bg-slate-100'}`}>Graf</button>
                                </div>
                                <div className="flex gap-3 text-[12px] font-black">
                                    X-Rubrik:<input className="w-20 border-b outline-none" value={el.xLabel} onChange={e=>setElements(p=>p.map(o=>o.id===el.id?{...o, xLabel:e.target.value}:o))} />
                                    Y-Rubrik:<input className="w-20 border-b outline-none" value={el.yLabel} onChange={e=>setElements(p=>p.map(o=>o.id===el.id?{...o, yLabel:e.target.value}:o))} />
                                </div>
                                <div className="max-h-32 overflow-y-auto">
                                    {el.rows.map((row, idx) => (
                                        <div key={idx} className="flex gap-2 mb-1">
                                            <input className="w-24 border-b text-xs" value={row.label} onChange={e => { const nr = [...el.rows]; nr[idx].label = e.target.value; setElements(p=>p.map(o=>o.id===el.id?{...o, rows: nr}:o)); }} />
                                            <input type="number" className="w-16 border-b text-xs" value={row.value} onChange={e => { const nr = [...el.rows]; nr[idx].value = parseFloat(e.target.value)||0; setElements(p=>p.map(o=>o.id===el.id?{...o, rows: nr}:o)); }} />
                                            <button onClick={()=>{ const nr = el.rows.filter((_, i)=>i!==idx); setElements(p=>p.map(o=>o.id===el.id?{...o, rows: nr}:o)); }} className="text-rose-500 font-bold ml-1">X</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {el.type === 'triangle' && (<>
                            <button onClick={()=>setElements(p=>p.map(o=>o.id===el.id?{...o, triangleType: 'right'}:o))} className="px-3 py-1 bg-slate-100 rounded text-xs">Rät</button>
                            <button onClick={()=>setElements(p=>p.map(o=>o.id===el.id?{...o, triangleType: 'isosceles'}:o))} className="px-3 py-1 bg-slate-100 rounded text-xs">Liksid</button>
                            <button onClick={()=>setElements(p=>p.map(o=>o.id===el.id?{...o, triangleType: 'scalene'}:o))} className="px-3 py-1 bg-slate-100 rounded text-xs">Olik</button>
                        </>)}
                        {el.type === 'ruler' && (<>
                            Min:<input type="number" className="w-16 border-b text-center text-lg" value={el.min} onChange={e=>setElements(p=>p.map(o=>o.id===el.id?{...o, min:parseInt(e.target.value)}:o))} />
                            Max:<input type="number" className="w-16 border-b text-center text-lg" value={el.max} onChange={e=>setElements(p=>p.map(o=>o.id===el.id?{...o, max:parseInt(e.target.value)}:o))} />
                            <select className="bg-slate-100 rounded-lg p-2" value={el.unitType} onChange={e=>{
                                const isFrac = e.target.value === 'fraction';
                                setElements(p=>p.map(o=>o.id===el.id?{...o, unitType:e.target.value, min:isFrac?0:o.min, max:isFrac?2:o.max}:o));
                            }}><option value="whole">Whole</option><option value="decimal">Decimal</option><option value="fraction">Fraction</option></select>
                            {el.unitType === 'fraction' && (
                                <select className="bg-slate-100 rounded-lg p-2" value={el.denom} onChange={e=>setElements(p=>p.map(o=>o.id===el.id?{...o, denom:parseInt(e.target.value)}:o))}>
                                    {[2,3,4,5,6,7,8,9,10].map(d => <option key={d} value={d}>1/{d}</option>)}
                                </select>
                            )}
                        </>)}
                        ...
                        {(el.divisions || el.type === 'spinner') && (<>
                            <button onClick={()=>updateDivisions(el.id, -1)} className="w-12 h-12 bg-slate-100 rounded-xl font-black text-2xl shadow-sm">-</button>
                            <span className="px-3 text-lg">{el.divisions} Delar</span>
                            <button onClick={()=>updateDivisions(el.id, 1)} className="w-12 h-12 bg-slate-100 rounded-xl font-black text-2xl shadow-sm">+</button>
                            {el.type === 'spinner' && <button onClick={()=>spinSpinner(el.id)} className="bg-emerald-500 text-white rounded-xl p-4 shadow-lg ml-2 hover:scale-110 transition-transform"><Play size={24} fill="white"/></button>}
