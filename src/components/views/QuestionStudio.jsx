...
export default function QuestionStudio({ 
  ...
  const t = {
    sv: {
      ...
      create_donow: "Starta Grid", publish: "Skriv ut / Presentera", title_label: "Namnge din session/blad",
      ...
      save_btn: "Spara", live_btn: "Digital Live", btn_close: "Stäng Studio"
    },
    en: {
      ...
      create_donow: "Start Grid", publish: "Print / Present", title_label: "Name your session/sheet",
      ...
      save_btn: "Save", live_btn: "Digital Live", btn_close: "Close Studio"
    }
  }[lang];
  ...
  const fetchLibrary = async () => {
    ...
    try {
        ...
        if (!user) return;
        const { data, error } = await supabase.from('saved_sheets').select('*').eq('user_id', user.id).order('updated_at', { ascending: false });
  ...
  const handleSave = async () => {
      ...
      try {
          ...
          const sheetData = { user_id: user.id, title: sheetTitle.substring(0, 100), type: setupMode, packet: packet, config: { showWorkArea, lang }, updated_at: new Date().toISOString() };
          let response;
          if (activeSheetId) { response = await supabase.from('saved_sheets').update(sheetData).eq('id', activeSheetId).select().single(); } 
          else { response = await supabase.from('saved_sheets').insert([sheetData]).select().single(); }
          if (response.error) throw response.error;
          setActiveSheetId(response.data.id); setIsSaved(true); alert(t.save_success); fetchLibrary(); 
  ...
  const loadSheet = (sheet) => {
      setPacket(sheet.packet); setSheetTitle(sheet.title); setSetupMode(sheet.type); setActiveSheetId(sheet.id); setIsSaved(true);
  ...
  const updatePacketItem = (id, key, val) => {
    setPacket(packet.map(p => p.id === id ? { ...p, [key]: val } : p)); setIsSaved(false);
  };
  ...
  const handleLaunchGrid = () => {
    if (!isSaved && !window.confirm(t.unsaved_warning)) return;
    onDoNowGenerate({ title: sheetTitle }, packet);
  };
  ...
  if (!setupMode) {
    return (
      <div className="flex-1 bg-slate-50 flex flex-col items-center p-12 overflow-y-auto custom-scrollbar relative">
        <button onClick={onClose} className="absolute top-8 right-8 p-3 bg-slate-900 text-white hover:bg-rose-600 rounded-2xl shadow-xl transition-all flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
            <X size={18}/> {t.btn_close}
        </button>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-100 font-sans overflow-hidden">
      {/* PERSISTENT HEADER BAR */}
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm z-50">
        <div className="flex items-center gap-4">
          <button onClick={() => { if(!isSaved && !window.confirm(t.unsaved_warning)) return; setSetupMode(null); }} className="text-[11px] font-black text-indigo-600 uppercase hover:underline flex items-center gap-1"><ChevronLeft size={16}/> {t.change_mode}</button>
          <div className="h-4 w-px bg-slate-200 mx-2"></div>
          <h2 className="text-sm font-black uppercase italic tracking-tight text-slate-900">{setupMode === 'worksheet' ? t.worksheet_title : t.donow_title}: {sheetTitle || "Nytt blad"}</h2>
        </div>
        <button onClick={onClose} className="p-2 bg-slate-900 text-white hover:bg-rose-600 rounded-xl shadow-lg transition-all flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`bg-white border-r border-slate-200 flex flex-col shrink-0 transition-all duration-300 ${isPane1Collapsed ? 'w-16' : 'w-72'}`}>
          <div className={`p-4 border-b flex items-center ${isPane1Collapsed ? 'justify-center' : 'justify-end'}`}>
        ...
        <div className="w-72 bg-white border-l border-slate-200 flex flex-col shadow-2xl shrink-0">
          ...
          {/* FACIT SETTINGS (Fixed Toggle & Dropdown) */}
          {setupMode === 'worksheet' && (
              <div className="p-6 border-t bg-white space-y-4">
                  <div className="flex items-center justify-between">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider cursor-pointer" onClick={() => setIncludeAnswerKey(!includeAnswerKey)}>{t.answer_key_toggle}</label>
                  ...
                  {includeAnswerKey && (
                      <div className="animate-in fade-in slide-in-from-top-2 border-t pt-4 border-slate-50">
                          <label className="text-[9px] font-black uppercase text-slate-300 mb-2 block">{t.answer_style_label}</label>
                          <select value={answerKeyStyle} onChange={(e) => setAnswerKeyStyle(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black uppercase px-3 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="p-6 border-t bg-slate-50 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                  ...
                  {setupMode === 'worksheet' ? (
                      ...
                      <button onClick={handleLaunchGrid} disabled={packet.length === 0} className="py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-lg hover:bg-indigo-500 transition-all disabled:opacity-50">
