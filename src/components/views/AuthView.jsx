...
export default function AuthView({ lang, studentMode, onSuccess, onBack, initialCode }) {
    ...
    const t = {
        sv: { 
            ...
            btn_join: "Gå med",
            error_code: "Ogiltig kod eller avslutat rum.",
        ...
        en: { 
            ...
            btn_join: "Join",
            error_code: "Invalid code or closed room.",
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            if (session?.user) {
                ...
                if (!profile || !profile.subscription_status || !profile.class_code) {
                    ...
                    setFullName(prev => (prev !== '' ? prev : session.user.user_metadata?.full_name || ''));
        ...
        try {
            ...
            if (studentMode === 'live') {
                ...
                const { data: room, error } = await supabase
                    .from('rooms')
                    .select('*')
                    .in('class_code', potentialMatches)
                    .eq('status', 'active')
                    .maybeSingle();

                if (error || !room) throw new Error(t.error_code);
                if (!fullName.trim()) throw new Error(t.name_placeholder);
                ...
                if (error || !teacher) throw new Error(t.error_code);

                onSuccess({ role: 'student', class: teacher });
            }
        } catch (err) {
    ...
    const startTrial = async () => {
        ...
        const classCodeStr = generateClassCode();
        try {
            const { error } = await supabase.from('profiles').update({ 
                subscription_status: 'trial',
                subscription_end_date: endDate.toISOString(),
                class_code: classCodeStr 
            }).eq('id', pendingUser.id);
            if (error) throw error;
                ...
                {studentMode ? (
                    <form onSubmit={handleStudentJoin} className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Kod</label>
                            <input 
                                type="text" 
                                maxLength={12} 
                                value={code} 
                                onChange={(e) => setCode(e.target.value.toUpperCase())} 
                                className="w-full p-6 rounded-[2rem] text-center text-3xl font-bold uppercase tracking-[0.2em] outline-none transition-all border-4 bg-amber-50/30 border-amber-50 focus:border-amber-500 focus:bg-white" 
                                placeholder="KOD" 
                                required 
                            />
                        </div>
                        ...
                        {studentMode === 'live' && (
                            <div className="space-y-2 animate-in slide-in-from-top-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">{t.name_label}</label>
                                <div className="relative">
                                    <User className="absolute left-5 top-5 text-emerald-200" size={18} />
                                    <input 
                                        type="text" 
                        ...
                        <button disabled={loading} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-bold text-xl uppercase tracking-widest shadow-xl hover:bg-emerald-600 active:scale-95 transition-all">
                            {loading ? <Loader2 className="animate-spin mx-auto" /> : t.btn_join}
                        </button>
                        
                        {message && (
                            <div className="p-4 rounded-2xl bg-rose-50 text-rose-600 border border-rose-100 text-xs font-bold text-center flex items-center justify-center gap-2">
