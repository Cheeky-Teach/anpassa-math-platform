...
export class ScaleGenerator {
    ...
    public static generate(level: number, seed: string, lang: Language = 'sv', multiplier: number = 1): GeneratedQuestion {
        ...
        // Use new random logic for levels 2-6
        // For Level 2 we might want to keep it slightly simpler, but the helper is fine.
        ...
        if (mode === 2) {
            ...
            return {
                ...
                renderData: { text_key: "calc_len_easy", description: descriptionObj, latex: scaleStr, answerType: 'numeric', geometry: { type: 'scale_single', shape, label: geomLabel }, variables: {} },
        ...
        if (mode === 3) {
            // Note: Mixed units usually imply Reduction (Map scales), 
            // but we can support Enlargement if we are careful with units.
            // For simplicity and realism, Level 3 focuses on standard map scales (1:X) mostly,
            // but let's stick to the requested randomness if possible.
            // Given "Mixed Units" usually implies large distances (m/km), 1:X is most logical.
            // We will stick to 1:X for Level 3 to avoid confusion with "microscopic" enlargement contexts.
        ...
        if (mode === 5) {
            ...
            const factor = ScaleGenerator.getScaleFactor(rng); // Broad range
            
            // Randomize Reduction vs Enlargement
            const isReduction = rng.intBetween(0, 1) === 1;
            ...
            if (isReduction) {
                ...
                descriptionObj = {
                    sv: `På en ritning är en ${svShape} ${drawVal} cm lång. I verkligheten är den ${showM ? realM + ' m' : realVal + ' cm'}. Vad är skalan (i cm)?`,
                    en: `On a drawing, a ${enShape} is ${drawVal} cm long. In reality it is ${showM ? realM + ' m' : realVal + ' cm'}. What is the scale (in cm)?`
                };
        ...
        if (mode === 6) {
            ...
            // Randomize Reduction vs Enlargement for Types 3 & 4
            const isReduction = rng.intBetween(0, 1) === 1;
            ...
            if (subType === 1) { // Find Scale (Visual) - Always Reduction for visual simplicity
            ...
            else if (subType === 3) { // Length -> Area Scale (Supported X:1)
            ...
            else { // Area -> Length Scale (Supported X:1)
